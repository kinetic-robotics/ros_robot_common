...
#include "friction_controller/friction_effort_controller.h"
#include <robot_toolbox/tool.h>

namespace friction_controller
{
FrictionEffortController::FrictionEffortController()
{
}

void FrictionEffortController::shotSpeedCallback(const robot_msgs::Float64StampedConstPtr& msg)
{
    targetShotSpeed_ = msg->result;
}

bool FrictionEffortController::init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle& node)
{
    /* 读取配置 */
    ...
    shotSpeedSubscriber_ = node.subscribe<robot_msgs::Float64Stamped>("speed/command", 1000, &FrictionEffortController::shotSpeedCallback, this);
    ROS_INFO("Friction Effort Controller started.");
    return true;
}

void FrictionEffortController::update(const ros::Time& time, const ros::Duration& period)
{
    /* PID计算与发布话题 */
...
void FrictionEffortController::starting(const ros::Time& time)
{
}

void FrictionEffortController::stopping(const ros::Time& time)
{
    for (auto iter = joints_.begin();iter != joints_.end();iter++) {
...
PLUGINLIB_EXPORT_CLASS(friction_controller::FrictionEffortController, controller_interface::ControllerBase);