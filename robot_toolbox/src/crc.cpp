...
namespace robot_toolbox
{
    namespace CRC { 
        ...
        {
            const static uint8_t INIT = 0xff;
            ...
            for (size_t i = 0; i < data.size(); i++) {
                int ucIndex = ucCRC8 ^ data[i];
                ucCRC8      = TABLE[ucIndex];
            }
            return ucCRC8;
        }
        ...
        {
            const static uint16_t INIT       = 0xffff;
            ...
            int ucCRC16 = INIT;
            for (size_t i = 0; i < data.size(); i++) {
                ucCRC16 = ((uint16_t)ucCRC16 >> 8) ^ TABLE[((uint16_t)ucCRC16 ^ (uint16_t)data[i]) & 0x00ff];
            }
            return ucCRC16;
        }
