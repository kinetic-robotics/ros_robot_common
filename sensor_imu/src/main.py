...
class Sensor:
        ...
        if length:
            ...
            if len(reading)!=0:
                for i in range(0,len(reading)):
                    try:
                        self.Circleloop.enqueue(reading[i])
                    except:
    ...
    def getVersion(self):
        #Get version info
        output = [0x5a, 0x06, 0x01, 0xf1, 0x00, 0xd7] #0xd7 is CRC-8 value
    ...
    def getSN(self):
        #Get version info
        output = [0x5a, 0x06, 0x01, 0xf3, 0x00, 0x46] #0x46 is CRC-8 value
        while(self.serialIDLE_flag):
            time.sleep(0.01)
        self.serialIDLE_flag = 1
        try:
    ...
    def timerIMUCB(self,event):
        if self.gravity:
            output = [0x5a, 0x06, 0x01, 0x17, 0x00, 0xff] #0x33 is CRC-8 value
        else:
            output = [0x5a, 0x06, 0x01, 0x1b, 0x00, 0xff] #0x33 is CRC-8 value
        while(self.serialIDLE_flag):
            time.sleep(0.01)
    ...
    def timerMagCB(self,event):
        output = [0x5a, 0x06, 0x01, 0x19, 0x00, 0xff] #0x33 is CRC-8 value
        while(self.serialIDLE_flag):
            time.sleep(0.01)
        self.serialIDLE_flag = 3
        try:
